...
public class MainActivity extends Activity implements Runnable, TextView.OnEditorActionListener {

    private static final String TAG = "SerialChat";
    private final String strHelp="? visualizza help\n" +
            "typhoon esegue AT+ASACMOD=12,5,1,0\n" +
            "code<n> imposta codice radio a <n>\n" +
            "wrp<code> imposta codice radio a <code>\n" +
            "rrp legge codice radio\n" +
            "eed imposta EEPROM raido a default\n" +
            "tf imposta tx fisso\n" +
            "tc termina tx fisso\n" +
            "nav0 nasconde barra navigazione\n" +
            "nav1 visualizza barra navigazione\n" +
            "ts reset falsh radio a modo 12, codice 1 protocollo barebone\n" +
            "f spedisce un file di test\n" +
            "load carica il file remoto send_tda7.txt\n" +
            "update aggiorna file remoto send_tda7.txt\n" +
            "loopfile trasmette e riceve file remoto\n" +
            "termnr<n> imposta numero terminale a <n>\n" +
            "un<n> esegue <n> tx/rx spegnendo radio ogni 250 messaggi\n" +
            "uf<n> esegue <n> tx/rx spegnendo radio ad ogni messaggio\n" +
            "alrt visualizza dialog di test\n" +
            "h invia e riceve 100 messaggi di test\n" +
            "eeras visualizza tempo auto spegnimento\n" +
            "eewas<n> imposta  tempo auto spegnimento a <n> minuti\n" +
            "d abilita disabilita modo diagnostico\n" +
            "ver visualizza versione firmware NOR\n" +
            "i info modulo NOR\n" +
            "a visualizza stato radio\n" +
            "s salva parametri radio\n" +
            "r reset modulo radio\n";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (false)
        {
            EEPROM eeprom = (EEPROM)getSystemService(MainActivity.EEPROM_SERVICE);
            if (!eeprom.exists())
            {
...
//     0 ..10239 	no password required 		no password required	read-all, write-all
        ...
        if (b_do_radio_service_test){
            ...
            RadioRF radioRF = (RadioRF)getSystemService(MainActivity.RADIORF_SERVICE);
...
//                       asac_radio_module_commands.parameters.i_crypt_code contiene codice crypt
        ...
        // if the command is executed successfully, we save the new setting in the radio module EEPROM and the radio module will reset
            ...
            String mytext = "TDA7 chat ver. "+ version + "\n"+ strHelp;
            m.obj = mytext;
            mHandler.sendMessage(m);
        }
...
// not a very useful comment here, and long too
...
// AT+ASACCHANNEL=40,0
        ...
        // eeras
        else if (text_input.startsWith("eeras"))
        {
            Asac_EEPROM ae = new Asac_EEPROM(this);
            ...
            AlertDialog alertDialog = asAlert.create();
            alertDialog.show();
        }
        //eewas<n>
        else if (text_input.startsWith("eewas"))
        {
            Asac_EEPROM ae = new Asac_EEPROM(this);
            int minutes = Integer.parseInt(text_input.substring(5));
            ae.writeTDA7_auto_shutdown(minutes);
